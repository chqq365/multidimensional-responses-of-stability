...
trend.sig<-c()
for ( s in unique(s_dd3$site_code)){
  # s<-"ahth.is"
  temp_s<- s_dd3%>%filter(site_code==s)
  
  test.result<-notrend_test(temp_s$spei, test = "MK") # monotonic trend (Mann–Kendall test)
  test.result1<-data.frame(site_code=s, p.value=test.result$p.value)
  trend.sig<-trend.sig%>%bind_rows(test.result1)
}
trend.sig1<-trend.sig%>%filter(p.value <= 0.05) 
# almost half of the sites have significant trends 

detrend.spei<-c()
for ( s in unique(s_dd3$site_code)){
  # s<-"ahth.is"
  temp_s<- s_dd3%>%filter(site_code==s)%>%arrange(year1)
  # fit a simple linear model 
   mod<-lm(spei~year1, data=temp_s)
   #plot(mod)
   resid.data<-residuals(mod)
   resid.data1<-resid.data%>%as.data.frame()%>%mutate(site_code=s, year1=temp_s$year1)
   detrend.spei<-detrend.spei%>%bind_rows(resid.data1)
}
colnames(detrend.spei)[1]<-c("spei")

## check again the distribution of the data 
ggplot(detrend.spei, aes(year1, spei)) + theme_bw() + geom_point() +
  facet_wrap(~site_code, scale="free_y", ncol=5)+
  geom_hline(yintercept = 0, linetype="dashed", color="red")

## add the start and end of the experiments 
s_dd4 <- merge(detrend.spei, gs4[,c("site_code", "site_fyear", "site_lyear")], by=c("site_code")) 
...
# ggsave(pp1, file="detrended SPEI based on cutoff 0.67 and 1.28 indicating years used.pdf", width = 21, height = 29.7, dpi=600)
...
# ggsave(check.year.0.67, file="years used for resistance and recovery based on cutoff 0.67_detrended spei.pdf", width = 21, height = 29.7, dpi=600)
...
for(i in c(1.28, 0.67)){ 
  ...
  ggsave(pp.trt.effects, height=4, width=8.26, dpi=600, file=paste0("effects on stability facets for ", i , "_detrended spei.pdf"))
  ggsave(pp.trt.effects, height=4, width=8.26, dpi=600, file=paste0("effects on stability facets for ", i , "_detrended spei.png"))
   }
...
for(i in unique(facet_data_cor_stab$id1)){ 
  ...
  ggsave(pp.cor2, height=4, width=8.26, dpi=600, file=paste0("relationships between stability facets ", i , "_with detrended spei.pdf"))
  ggsave(pp.cor2, height=4, width=8.26, dpi=600, file=paste0("relationships between stability facets ", i , "_with detrended spei.png"))
   }
...
for(i in unique(Aspect_data_cor$id1)){ 
  ...
  ggsave(pp.asp2,  height=4, width=8.26, dpi=600, file=paste0("relationships between community Aspects ", i , "_detrended spei.pdf"))
  ggsave(pp.asp2,  height=4, width=8.26, dpi=600, file=paste0("relationships between community Aspects ", i , "_detrended spei.png"))
  }
# the end 