...
# delete sites that do not have one extreme or normal year 
  ...
  s_dd7_select<-s_dd7 %>%filter(site_code %in%num.year.at.least.10$site_code)%>%filter(year_trt>0) %>%filter(site_code %in%s_dd7_check.extreme$site_code)
  
  data.bio.rich<-d8%>%filter(site_code %in%num.year.at.least.10$site_code)%>%
  ...
  d.select.com<-d8%>%filter(site_code %in%num.year.at.least.10$site_code)%>%
